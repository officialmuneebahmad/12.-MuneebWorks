document.addEventListener('DOMContentLoaded', () => {
  ...
  // Chart.js Standards Graph
  const ctx = document.getElementById('standards-chart');
  if (ctx) {
    const isDarkMode = document.body.classList.contains('dark-mode');
    const getTextColor = () => document.body.classList.contains('dark-mode') ? '#B3A79B' : '#7A6F63';
    const getGridColor = () => document.body.classList.contains('dark-mode') ? 'rgba(255, 255, 255, 0.05)' : 'rgba(31, 24, 16, 0.05)';

    const data = {
      labels: ['Initial Audit', 'Code Minification', 'Image Compression', 'CDN & Caching', 'MW Standard'],
      datasets: [{
        label: 'Load Time',
        data: [5.4, 3.2, 1.8, 1.1, 0.6],
        borderColor: '#FF6B35',
    ...
    const standardsChart = new Chart(ctx, config);

    const themeToggleBtn = document.querySelector('.theme-toggle');
    if (themeToggleBtn) {
      themeToggleBtn.addEventListener('click', () => {
        setTimeout(() => {
          standardsChart.options.scales.x.ticks.color = getTextColor();
          standardsChart.options.scales.y.ticks.color = getTextColor();
          standardsChart.options.scales.y.grid.color = getGridColor();
